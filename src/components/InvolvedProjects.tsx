...
const InvolvedProjects = () => {
  return (
    <section
      className="bg-gradient-to-tr  from-black via-[#0a0a0a] to-[#0c001b] min-h-screen text-white pt-24 pb-32 px-6 sm:px-12 md:px-20"
      id="projects"
    >
      ...
      <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {projects.map((project, idx) => (
          ...
          >
            <div className="w-full h-full flex flex-col justify-between border border-purple-800 rounded-xl shadow-md bg-[#0f0f0f] p-4 hover:shadow-purple-800/50">
              <div>
              ...
              <div className="flex flex-wrap gap-2 text-xs text-gray-400 mt-auto">
                {project.tech.map((tech, i) => (
                  <span
                    key={i}
                    className="bg-purple-700 bg-opacity-20 border border-purple-700 rounded px-2 py-1"
                  >
                    {tech}
                  </span>
                ))}
      ...
      <div className=" justify-center items-center mt-6 ">
        <div className=" flex-1 transition-transform duration-300 transform hover:scale-105 border border-purple-800 rounded-xl shadow-md bg-[#0f0f0f] pt-8 hover:shadow-purple-800/50">
          <img
            src={QuestioningAvatar}
            alt="Questioning Avatar"
            className="w-52 sm:w-64 md:w-72 lg:w-80 h-auto ml-4"
          />
        </div>
      </div>
    </section>
